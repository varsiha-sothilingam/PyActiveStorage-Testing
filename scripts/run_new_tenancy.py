...
def test_https_file(maxThreads, slice):
    """
    ...
    test_file_uri = "https://gws-access.jasmin.ac.uk/public/canari/varsiha/cl_Amon_UKESM1-0-LL_ssp370SST-lowNTCF_r1i1p1f2_gn_205001-209912.nc"
    active = Active(test_file_uri, ncvar="cl",  active_storage_url=active_storage_url, option_disable_chunk_cache=True)
    active._version = 2
    active._method = "min"
    active._max_threads = maxThreads
  
    #result = active[0:40, 0:1920, 0:30, 0:30][0]
    #dataset cl: 600, 85, 144, 192
    result = active[slice:slice+20,:,:,:]#0:600,0:85,0:144,0:192]
    print(result)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--maxThreads",
        nargs=1,          # exactly 3 values
        type=int,         # convert them to integers
        help="Value for maximum number of threads"
    )
    parser.add_argument(
        "--slice",
        nargs=1,          # exactly 3 values
    ...
    print(args.maxThreads[0], args.slice[0])


    test_https_file(args.maxThreads[0], args.slice[0])